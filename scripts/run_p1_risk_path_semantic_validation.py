#!/usr/bin/env python3
"""Run P1 Risk vs Path Semantic Validation (analysis-only)."""
...
from chartai.analysis.p1_risk_path_semantic_validation import (
    ...
    format_risk_path_summary,
    save_risk_path_report,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="P1 Risk vs Path Validation")
    parser.add_argument("--csv", type=Path, default=Path("data/market/btcusdt_3m.csv"))
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("reports/p1_risk_path_semantic_validation_report.json"),
    )
    ...
    report = P1RiskPathSemanticValidationRunner(btc).run(test_pass_count=pass_count)
    print(format_risk_path_summary(report))

    args.report.parent.mkdir(parents=True, exist_ok=True)
    save_risk_path_report(report, str(args.report))
