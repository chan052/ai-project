#!/usr/bin/env python3
"""Run P1 Return/Risk Composite Validation experiment (analysis-only)."""
...
from chartai.analysis.p1_composite_validation_experiment import (
    ...
    format_composite_validation_summary,
    save_composite_validation_report,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="P1 Composite Validation")
    parser.add_argument("--csv", type=Path, default=Path("data/market/btcusdt_3m.csv"))
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("reports/p1_composite_validation_experiment_report.json"),
    )
    ...
    report = P1CompositeValidationRunner(btc).run(test_pass_count=pass_count)
    print(format_composite_validation_summary(report))

    args.report.parent.mkdir(parents=True, exist_ok=True)
    save_composite_validation_report(report, str(args.report))
