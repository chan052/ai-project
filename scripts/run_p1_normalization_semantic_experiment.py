#!/usr/bin/env python3
"""Run P1 Normalization Semantic Preservation experiment (analysis-only)."""
...
from chartai.analysis.p1_normalization_semantic_experiment import (
    ...
    format_norm_experiment_summary,
    save_norm_experiment_report,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="P1 Normalization Semantic Experiment")
    parser.add_argument("--csv", type=Path, default=Path("data/market/btcusdt_3m.csv"))
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("reports/p1_normalization_semantic_experiment_report.json"),
    )
    ...
    report = P1NormalizationSemanticExperimentRunner(btc).run(test_pass_count=pass_count)
    print(format_norm_experiment_summary(report))

    args.report.parent.mkdir(parents=True, exist_ok=True)
    save_norm_experiment_report(report, str(args.report))
