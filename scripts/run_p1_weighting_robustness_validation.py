#!/usr/bin/env python3
"""Run P1 Return/Risk Weighting Robustness Validation (analysis-only)."""
...
from chartai.analysis.p1_weighting_robustness_validation import (
    ...
    format_weighting_robustness_summary,
    save_p1_weighting_robustness_report,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="P1 Weighting Robustness Validation")
    parser.add_argument("--csv", type=Path, default=Path("data/market/btcusdt_3m.csv"))
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("reports/p1_weighting_robustness_validation_report.json"),
    )
    ...
    report = P1WeightingRobustnessRunner(btc).run(test_pass_count=pass_count)
    print(format_weighting_robustness_summary(report))

    args.report.parent.mkdir(parents=True, exist_ok=True)
    save_p1_weighting_robustness_report(report, str(args.report))
